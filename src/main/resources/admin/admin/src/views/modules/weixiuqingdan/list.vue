		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"150px","margin":"0","position":"absolute","right":"7%","background":"none","zIndex":"1003"}' :inline="true" :model="searchForm">
				<el-row :style='{"flexWrap":"wrap","justifyContent":"center","display":"flex"}' >
					<div :style='{"width":"100%","margin":"0 0 10px","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
						<label :style='{"margin":"0","color":"#333","textAlign":"left","display":"inline-block","width":"100%","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">报修标题</label>
						<el-input v-model="searchForm.baoxiubiaoti" placeholder="报修标题" clearable></el-input>
					</div>
					<div :style='{"width":"100%","margin":"0 0 10px","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
						<label :style='{"margin":"0","color":"#333","textAlign":"left","display":"inline-block","width":"100%","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">业主账号</label>
						<el-input v-model="searchForm.yezhuzhanghao" placeholder="业主账号" clearable></el-input>
					</div>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="shigongtupian" width="200" label="施工图片">
						<template slot-scope="scope">
							<div v-if="scope.row.shigongtupian">
								<img v-if="scope.row.shigongtupian.substring(0,4)=='http'" :src="scope.row.shigongtupian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.shigongtupian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('weixiuqingdan','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' v-if="isAuth('weixiuqingdan','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"2px solid","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(135, 154, 108, 1)","borderRadius":"4px","background":"#fff","borderImage":"linear-gradient(180deg, rgba(135.00000715255737, 154.00000602006912, 108.00000116229057, 1), rgba(226.0000017285347, 226.0000017285347, 226.0000017285347, 0.3799999952316284)) 1 1","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('weixiuqingdan','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"2px solid","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(135, 154, 108, 1)","borderRadius":"4px","background":"#fff","borderImage":"linear-gradient(180deg, rgba(135.00000715255737, 154.00000602006912, 108.00000116229057, 1), rgba(226.0000017285347, 226.0000017285347, 226.0000017285347, 0.3799999952316284)) 1 1","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('weixiuqingdan','维修评价')" type="success" size="mini" @click="weixiupingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','','')">维修评价</el-button>
...
export default {
  ...
  methods: {
	...
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.$storage.set('crossTable','weixiuqingdan');
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          shigongtupian: row.shigongtupian,
          weixiujine: row.weixiujine,
          guzhangbaogao: row.guzhangbaogao,
          weixiuxiangqing: row.weixiuxiangqing,
          wangongshijian: row.wangongshijian,
